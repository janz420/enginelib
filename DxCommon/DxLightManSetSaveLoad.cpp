...
//XMEN2 | 15-02-2013 | add new wld version
void	DXLIGHT::LoadSet( CSerialFile &SFile )
{
	...
	if ( VERSION==dwVer )
	{
		...
		SFile.ReadBuffer ( m_szLightName, sizeof ( char ) * MAX_PATH );
		SFile >> m_bEnable;
		SFile >> m_BaseDiffuse;
...
void	DXLIGHT::LoadSet_Ver104( CSerialFile &SFile )
{
	BOOL	bResult = FALSE;
...
void DxLightMan::Load ( CSerialFile &SFile, DWORD dwLandVer, BOOL bSETLAND )
{
	...
	else
	{
		DWORD			dwVersion;
		float			fTemp;
		D3DXVECTOR3		m_vDumMax;
		D3DXVECTOR3		m_vDumMin;
		SFile >> dwVersion;
		...
		else if( 0x00104==dwVersion )
		{
			...
			m_LtDirectNoon.LoadSet_Ver104 ( SFile );
			m_LtDirectNight.LoadSet_Ver104 ( SFile );

			SFile >> bResult;
			if ( bResult )
			{
				m_pLtListHead = new DXLIGHT();
				m_pLtListHead->LoadSet_Ver104 ( SFile );
			}
