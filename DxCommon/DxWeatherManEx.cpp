...
const	DWORD	DxEffectRain::OLDVERSION	=	0x103;
...
{
	DWORD	dwSize = sizeof(RAIN_PROPERTY_103);
	SFile << (DWORD) OLDVERSION;
	SFile << dwSize;

	SFile.WriteBuffer ( &m_Property, sizeof(RAIN_PROPERTY_103) );
}
...
const	DWORD	DxEffectCloud::OLDVERSION	=	0x100;
//XMEN2 | 15-02-2013 | add new wld version
...
{
	DWORD	dwSize = sizeof(CLOUD_PROPERTY_100);
	SFile << (DWORD) OLDVERSION;
	SFile << dwSize;

	SFile.WriteBuffer ( &m_Property, sizeof(CLOUD_PROPERTY_100) );
}
