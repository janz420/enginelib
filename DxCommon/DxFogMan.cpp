...
{
	SFile << OLDVERSION;//add by alex 20131205
...
void	DxFogMan::RenderFogSB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !m_bManualRange )//lev톝펃
	{
		if ( fWeatherRate > 0.f )	fFogEnd -= 50.f*fWeatherRate;			// ?? ?? ?? ??? ???.
		if ( fBlendFact < 1.f )		fFogEnd -= 50.f*(1.f-fBlendFact);		// ???? ??? ???.
	}
...
void	DxFogMan::SetManualRange( float fVal )//lev톝펃
{
	//this function only modify high range

	m_fRangeHigh_Far = fVal;
	m_bManualRange = TRUE;
}
