...
BOOL DxSkinVehicleData::SaveFile ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	/*SFile << m_fScale;*/ //added by sid | 3-3-15 | Official VCF Reading
	...
	//added by sid | 3-3-15 | Official VCF Reading
	SFile << m_fScale;
	SFile << m_dwBONE;
...
BOOL DxSkinVehicleData::LoadFile ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread, bool bREPRESH )
{
	...
	{
		BOOL bLoad = DxSkinAniControl::LoadAnimation ( m_vecANIFILE[i].c_str(), pd3dDevice, bREPRESH );//, true );
	}
