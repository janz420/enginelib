...
DxSkinChar::DxSkinChar(void) :
	...
	m_bWorldObj(FALSE),
	m_dwVertexNUM(0),
	m_dwBONE(0), //added by XMEN2 | 18-7-2012 | Add ABL and ABF code
	m_bBigHead(FALSE),
	//Added by XMEN2 | 11-8-2012 | add VPS and VCF code
	m_pSkinVehicle(NULL),
	m_fLengthSRC(0.f)
{
	memset( m_szFileName, 0, FILE_LENGTH );

	//added by XMEN2 | 18-7-2012 | Add ABL and ABF code
...
HRESULT DxSkinChar::Clear( EMPIECECHAR emType )
{
	...
	//added by XMEN2 | 14-9-2012 | test dual cps on
	if ( m_PartArray[emType].GetFileName() )
	{
		m_PartArray[emType].ReSetPart();
	}
...
HRESULT DxSkinChar::SetPiece ( const TCHAR* szFile, LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwFlag, int nLevel, bool bCharacterData /*= FALSE*/ )
{
	...
	if ( pSkinPiece->m_emType == PIECE_RHAND || pSkinPiece->m_emType == PIECE_LHAND )
	{
		...
		//RIGHT
		//if ( m_pSkinCharPiece[PIECE_RHAND] )
		//	m_pSkinCharPiece[PIECE_RHAND]->ClearAll();
	...
	if ( pSkinPiece->m_emType == PIECE_REBOOT_RHAND || pSkinPiece->m_emType == PIECE_REBOOT_LHAND )
	{
		//LEFT
		m_strBoneLink[PIECE_REBOOT_LHAND] = "";
		m_dwWeaponSlot[PIECE_REBOOT_LHAND] = 0;
	...
	if ( pSkinPiece->m_emType == PIECE_REBOOT_RHAND )
	{
		if ( m_PartArray[PIECE_REBOOT_LHAND].GetFileName() )
		{
			m_PartArray[PIECE_REBOOT_LHAND].ReSetPart ();
		}
	}
	else
	{
...
void DxSkinChar::SetColorAll( WORD wColor ) // Added By Janzs420
{
	WORD wR, wG, wB;
	wR = wColor >> 10;
	wG = wColor >> 5;
...
void DxSkinChar::ResetPiece ( int i )
{
	...
	else if ( !m_pSkinCharPiece[PIECE_REBOOT_RHAND] )
	{
		SAFE_DELETE( m_pSkinCharPiece[ PIECE_REBOOT_LHAND ] );
		m_strBoneLink[PIECE_REBOOT_LHAND] = "";
		m_dwWeaponSlot[PIECE_REBOOT_LHAND] = 0;
	...
	if ( i == PIECE_RHAND )
	{
		if ( m_PartArray[PIECE_RHAND].GetFileName() ) //&& m_PartArray[PIECE_LHAND].GetFileName() )
		{
			m_PartArray[PIECE_RHAND].ReSetPart ();
			m_PartArray[PIECE_LHAND].ReSetPart ();
		}
	}
	else 
	...
	if ( i == PIECE_REBOOT_RHAND )
	{
		if ( m_PartArray[PIECE_REBOOT_RHAND].GetFileName() ) 
		{
			m_PartArray[PIECE_REBOOT_RHAND].ReSetPart ();
			m_PartArray[PIECE_REBOOT_LHAND].ReSetPart ();
		}
	}
	else 
...
//added by XMEN2 | 18-7-2012 | Add ABL and ABF code
BOOL DxSkinChar::SetCharPieceData( DxSkinPieceData* pCharData, LPDIRECT3DDEVICEQ pd3dDevice , std::string szBoneLink , DWORD WeaponSlot , DWORD PieceType , DWORD dwFlag, int nLevel, bool bCharacterData )
{
	if ( !pCharData || PieceType > PIECE_SIZE ) return FALSE;

	//XMEN2 | fix abl
	m_PartArray[PieceType].ReSetPart ();
	/////
...
HRESULT DxSkinChar::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX &matRot, const BOOL bShadow, const BOOL bEff, const BOOL bReverse )
{
	...
	//Added by XMEN2 | 18-7-2012 | add mouse effect code
