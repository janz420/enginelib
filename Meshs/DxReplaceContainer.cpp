...
HRESULT DXREPLACEPIECE::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pMatWorld, CLIPVOLUME* pCV, float fAlpha )
{
	...
	else if ( m_pStaticMesh )
	{
		if( pCV )
		{
			...
			m_pStaticMesh->Render_DYNAMIC_0( pd3dDevice, sCV, NULL );
			m_pStaticMesh->Render_DYNAMIC_1( pd3dDevice, sCV, NULL );
			m_pStaticMesh->Render_DYNAMIC_2( pd3dDevice, sCV, NULL );
