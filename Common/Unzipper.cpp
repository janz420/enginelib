...
UINT CUnzipper::UnzipToMemory( LPCTSTR szZipFileName, LPCTSTR szFileName, PBYTE &cBuffer )
{
	...
	if( GOTOFILEPOS( strFileName ) ) // GotoFilePos�� �̵�
	{
		UZ_FileInfo info;
		GetFileInfo(info);
		...
		if( IsEncrypted( info.dwFlags ) ) // MEMO : ��ȣȭ �� �����̸�...
		{
			if( unzOpenCurrentFilePassword(m_uzFile, "tkkfb") != UNZ_OK )
				return UINT_MAX;
		}
		else
		{
		...
		UINT nRet(0);
		char pBuffer[BUFFERSIZE] = "";
		cBuffer = new BYTE[info.dwUncompressedSize]; // ȣ���ϴ� �κп��� �޸� ����
